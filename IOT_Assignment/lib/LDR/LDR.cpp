 ...
 */
 ...
 LDR::LDR(int pin, unsigned long other_resistor, ePhotoCellKind kind, unsigned int adc_resolution_bits, unsigned int smoothing_history_size) :
   _pin (pin),
 ...
 LDR::LDR(int pin, unsigned long other_resistor, float mult_value, float pow_value, unsigned int adc_resolution_bits, unsigned int smoothing_history_size) :
   _pin (pin),
   _other_resistor (other_resistor),
 ...
 void LDR::updatePhotocellParameters(float mult_value, float pow_value)
 {
   _mult_value = mult_value;
   _pow_value = pow_value;
 }
 
 float LDR::luxToFootCandles(float intensity_in_lux)
 {
   return intensity_in_lux/10.764;
 }
 
 float LDR::footCandlesToLux(float intensity_in_footcandles)
 {
   return 10.764*intensity_in_footcandles;
 }
 
 void LDR::setPhotocellPositionOnGround(bool on_ground)
 {
   _photocell_on_ground = on_ground;
 }
 
 int LDR::getCurrentRawAnalogValue() const
 {
 ...
 float LDR::rawAnalogValueToLux(int raw_analog_value) const
 {
 ...
 float LDR::getCurrentLux() const
 {
   return rawAnalogValueToLux(getCurrentRawAnalogValue());
 }
 
 float LDR::getCurrentFootCandles() const
 {
   return luxToFootCandles(getCurrentLux());
 }
 
 float LDR::getSmoothedLux()
 {
   float sumResult = 0;
 ...
 float LDR::getSmoothedFootCandles()
 {
